...
def normalize_answer(answer):
    ...
    if answer in ["✅", "对", "正确", "√", "true", "True", "T", "t", "是", "yes", "Yes"]:
        return "对"
    elif answer in ["❌", "错", "错误", "×", "false", "False", "F", "f", "否", "no", "No"]:
        return "错"
    return answer
...
def check_answer(user_input, question):
    ...
    if q_type == "判断":
        ...
        # 提取用户答案中的选项标签（支持多种格式）
        user_match = re.match(r'^[\(（\s]*([A-Da-d1-4])[\)）\s]*[\.．:\s]*', user_input)
        correct_match = re.match(r'^[\(（\s]*([A-Da-d1-4])[\)）\s]*[\.．:\s]*', correct_disp)

        if user_match and correct_match:
            # 转换为大写比较
            return user_match.group(1).upper() == correct_match.group(1).upper()
        else:
            # 如果没有匹配到选项格式，直接比较标准化后的答案
...
def intelligent_detect_question_type(question_text, correct_answer, options_text, explicit_type=None):
    ...
    # 改进的模式匹配：允许空格和各种标点
    choice_patterns = [
        r'[A-Da-d][\.．、:：]\s*[^\s]+',  # A. 内容 或 A、内容
        r'选项[ABCDabcd][\.．、:：]?\s*[^\s]+',  # 选项A. 内容
        r'[①②③④][\.．、:：]\s*[^\s]+',  # ①. 内容
        r'[1-4][\.．、:：]\s*[^\s]+',  # 1. 内容
    ]
...
if st.session_state.get("view_wrong_questions", False):
    ...
    if not wrong_questions:
        ...
        if idx < len(wrong_questions):
            ...
            with col1:
                st.markdown("**你的答案：**")
                st.error(wq.get('user_answer', ''))

            with col2:
                st.markdown("**正确答案：**")
                st.success(wq.get('correct_answer', ''))

            # 解析
            if wq.get('explanation'):
                st.markdown("**解析：**")
                st.info(wq.get('explanation'))

            # 复习次数
            review_count = wq.get('attempt_count', 0)
            st.caption(f"已复习次数：{review_count}次")
            ...
            with col1:
                if st.button("✅ 我已掌握", type="primary", use_container_width=True):
                    ...
                    wq['reviewed'] = True
                    # 从当前列表中移除
                    st.session_state.wrong_questions_list = [q for q in wrong_questions if
                                                             q.get('question_id') != wq.get('question_id')]
                    if st.session_state.wrong_question_index >= len(st.session_state.wrong_questions_list):
                        st.session_state.wrong_question_index = 0
                    st.success("已标记为已掌握！")
                    st.rerun()
        ...
        if st.session_state.all_questions and st.session_state.detection_stats:
            ...
            with col2:
                ...
                if saved_progress:
                    ...
                    # 计算答题统计
                    answered_questions = []
                    for idx, record in saved_progress.items():
                        if record.get("answer"):
                            answered_questions.append(idx)
                ...
                for i in range(row, end_idx):
                    ...
                    if q_progress.get("answer"):
                        if q_progress.get("correct", False):
                            question_status = "✅"  # 回答正确
                        else:
                            question_status = "❌"  # 回答错误
                    else:
                        question_status = "○"  # 未作答
